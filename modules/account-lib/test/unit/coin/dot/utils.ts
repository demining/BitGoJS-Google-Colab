...
describe('utils', () => {
  const Singleton = require('../../../../src/coin/dot/singletonRegistry');
  const singletonRegistry = Singleton.getInstance(material);
  ...
  it('should recover signature from raw tx correctly', () => {
    should.equal(
      utils.recoverSignatureFromRawTx(rawTx.transfer.signed, { registry: singletonRegistry }),
  ...
  it('should serialize signed transaction successfully', () => {
    const instance = Singleton.getInstance(material);
    const signature = utils.recoverSignatureFromRawTx(rawTx.transfer.signed, { registry: instance });
